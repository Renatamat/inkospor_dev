...
export const initHomepageQualityAnimations = (
  root: ParentNode = document,
): void => {
  if (typeof document === "undefined") return;

  root.querySelectorAll<HTMLElement>(".hp-quality-image").forEach((image) => {
  ...
  root.querySelectorAll<HTMLElement>(".hp-quality-slider").forEach((slider) => {
    if (initializedSliders.has(slider)) return;

    const cards = Array.from(
      slider.querySelectorAll<HTMLElement>(".hp-quality-item"),
    );
