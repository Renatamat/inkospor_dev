...
function FloatSidebar(options) {
  ...
  let init = () => {
    ...
    $sideOuter.style.position      = 'relative';

    $sideInner.style.width      = '100%';
    const computedWidth = $sideInner.getBoundingClientRect().width;
    $sideInner.style.width = `${computedWidth}px`;
    $sideInner.style.maxWidth = `${computedWidth}px`;
